...
- **Preisfokus präsent (27.04.2026):** In `STEP_EDIT` steht „Preis pro Gericht“ jetzt oberhalb des Preisfelds; das Eingabefeld selbst ist deutlich prominenter (größere Zahl, klarer Fokus-Rahmen, gelbe Wert-Fläche), damit der Verkaufspreis als zentrales Business-Element sofort sichtbar ist.
- **Preisfokus Ultra-Premium (27.04.2026):** Step 1 hebt den Verkaufspreis jetzt als Hero-Element hervor (`DEIN PREIS`-Label, größere Preis-Typo, stärkere Preis-Card). Ziel: Der Anbieter erkennt auf den ersten Blick, dass dies sein zentraler Erlöshebel ist.
- **Step2 Header-Polish + CTA-State-Fix (27.04.2026):** Der Badge `DEIN INSERAT` in der oberen Karte wurde entfernt und als stilvolle Überschrift oberhalb der Karte neu gesetzt (`Dein Inserat` + klare Aktivierungs-Headline). Zusätzlich nutzt der Publish-CTA jetzt den aktuell gewählten Step2-State robust (inkl. Guard gegen State-Desync), damit `Küche entlasten` / `Jetzt für 4,99 € inserieren` zuverlässig die richtige Ziel-Option öffnen.
- **Step2 Text-Finish (27.04.2026):** Headline und Zwischenzeile wurden auf einen hochwertigeren App-Ton geschärft (`So gehst du heute live`, `Wähle dein Modell`) für klarere Führung ohne Badge-Optik.
- **Step2 Kontext-Textlogik Wochenplan (27.04.2026):** Im Wochenplan-Kontext (`entryPoint='week'` oder `bulkDraftDates`) nutzt Step 2 jetzt eigene Texte (Kicker/Headline/Zwischenzeile + CTA-Label), damit Planungstexte nicht mit Dashboard-„Jetzt inserieren“-Texten vermischt werden.
- **Step2 Wochenplan-CTA Guard (27.04.2026):** Die Publish-Logik für Bulk/Wochenplan liest den Live-Toggle-State und prüft zusätzlich den CTA-Label-Text (`Woche stressfrei aktivieren`), damit auch bei kurzzeitigen State-Desyncs immer die richtige Aktivierungsoption ausgelöst wird.
- **Step2 Kacheltexte nach Kontext (27.04.2026):** Die beiden Auswahlkacheln sind jetzt ebenfalls kontextsensitiv formuliert: im Wochenplan-Modus sprechen Titel/Unterzeilen von Wochenaktivierung statt Einzel-Inserat, während im Dashboard-Modus die Einzelinserat-Sprache erhalten bleibt.
- **Monetarisierung-Boost Paket (27.04.2026):** Step 1/2 wurden auf schnellere Preisentscheidung und klarere Kostenwirkung erweitert: (1) Preis-Stepper `− / +` in 0,50er-Schritten, (2) Erlös-Schnellrechner unter dem Preis (`20 Portionen`-Vorschau), (3) Preisanker-Hinweis („Typisch in deiner Umgebung …“), (4) zusätzliche Konsequenzzeilen je Step2-Option (Fixkosten vs. erfolgsbasiert), (5) Wochenplan-Vergleichszeile (Fixkosten vs. Stressfrei-Modell), (6) CTA-Microfeedback beim Tippen (`Wird vorbereitet…`), um „nichts passiert“-Eindruck zu vermeiden.
